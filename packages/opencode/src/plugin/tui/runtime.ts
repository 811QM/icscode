...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".icscode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".icscode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".icscode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.ICSCODE_PURE ? [] : pluginOrigins
    if (Flag.ICSCODE_PURE && pluginOrigins.length) {
    }
