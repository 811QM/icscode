...
export async function checkPluginCompatibility(target: string, icscodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(icscodeVersion) || semver.major(icscodeVersion) === 0) return
  ...
  if (!semver.satisfies(icscodeVersion, range)) {
    throw new Error(`Plugin requires icscode ${range} but running ${icscodeVersion}`)
  }
}
