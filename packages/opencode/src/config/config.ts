...
function globalConfigFile() {
  const candidates = ["icscode.jsonc", "icscode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.ICSCODE_CONFIG && !Flag.ICSCODE_CONFIG_DIR && !Flag.ICSCODE_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "icscode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "icscode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "ICSCODE_CONFIG_CONTENT") return "local"
        ...
        if (Flag.ICSCODE_CONFIG) {
          yield* merge(Flag.ICSCODE_CONFIG, yield* loadFile(Flag.ICSCODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.ICSCODE_CONFIG })
        }

        if (!Flag.ICSCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("icscode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.ICSCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from ICSCODE_CONFIG_DIR", { path: Flag.ICSCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".icscode") || dir === Flag.ICSCODE_CONFIG_DIR) {
            for (const file of ["icscode.json", "icscode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.icscode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.ICSCODE_CONFIG_CONTENT) {
          const source = "ICSCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.ICSCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from ICSCODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["ICSCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("ICSCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.ICSCODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ICSCODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("ICSCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.ICSCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.ICSCODE_DISABLE_PRUNE) {
