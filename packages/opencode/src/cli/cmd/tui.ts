...
declare global {
  const ICSCODE_WORKER_PATH: string
}
...
async function target() {
  if (typeof ICSCODE_WORKER_PATH !== "undefined") return ICSCODE_WORKER_PATH
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start icscode tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start icscode in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
