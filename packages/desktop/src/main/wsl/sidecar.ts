...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export ICSCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export ICSCODE_CLIENT=desktop",
    `export ICSCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export ICSCODE_SERVER_PASSWORD=${shellEscape(password)}`,
