...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@icscode-ai/sdk/v2"
...
const CUSTOM_PROVIDER_OPTION_VALUE = "__ICSCODE_custom_provider__"
    ...
    ...pipe(
      ...
      map((provider) => ({
        ...
        description: {
          ...
          "icscode-go": "Low cost subscription for everyone",
        }[provider.id],
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in icscode.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
        ...
        ({
          icscode: (
            <box gap={1}>
              ...
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://icscode.ai/zen</span> to get a key
              </text>
            </box>
          ),
          "icscode-go": (
            <box gap={1}>
              <text fg={theme.textMuted}>
                icscode Go is a $10 per month subscription that provides reliable access to popular open coding models
                with generous usage limits.
              </text>
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://icscode.ai/go</span> and enable icscode Go
              </text>
            </box>
          ),
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in icscode.json to use it.`,
          })
          dialog.clear()
          return
        }
