import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@icscode-ai/plugin/tui"
...
import type { useicscodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useicscodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.geticscodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.geticscodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
