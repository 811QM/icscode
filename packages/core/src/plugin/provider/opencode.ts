...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load icscode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.catalog.transform((catalog) => {
      ...
      const item = catalog.provider.get(ProviderV2.ID.icscode)
      if (!item) return
      const hasKey = Boolean(process.env.ICSCODE_API_KEY || connected || item.provider.request.body.apiKey)
